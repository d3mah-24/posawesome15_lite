...
#             from .validate_return import validate_return_items
...
"""
Update Invoice Function
Handles invoice updates only (not creation)
"""
...
def update_invoice(data):
    """
    PUT - Update existing invoice data
    
    This function is called ONLY for updating existing invoices.
    For creating new invoices, use create_invoice instead.
    
    Features:
    - Updates existing Sales Invoice document
    - Handles item additions, updates, deletions
    - Deletes invoice if all items removed
    - Natural ERPNext document lifecycle
    """
    try:
        data = json.loads(data) if isinstance(data, str) else data
    ...
    try:
        # Ensure we have an invoice name
        if not data.get("name"):
            frappe.throw(_("Invoice name is required for updates. Use create_invoice for new invoices."))
        ...
        try:
            ...
            frappe.throw(_("Invoice {0} does not exist. Use create_invoice for new invoices.").format(data.get("name")))

        # Basic validation for return invoices
        is_return_invoice = data.get("is_return") or invoice_doc.is_return
        if is_return_invoice and invoice_doc.get("return_against"):
            from .validate_return import validate_return_items
            invoice_items = [d.as_dict() for d in invoice_doc.items]
            validation = validate_return_items(
        ...
        try:
            ...
            # Don't fail update if offers fail
            frappe.logger().error(f"Error applying offers during update: {str(e)}")
