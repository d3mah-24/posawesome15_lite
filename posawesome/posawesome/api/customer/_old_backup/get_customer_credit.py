...
def get_customer_credit_summary(customer_id, company=None):
    ...
    try:
        ...
        # Get invoice credits total
        invoice_credit = frappe.db.sql("""
            SELECT COALESCE(SUM(ABS(outstanding_amount)), 0) as total
        ...
        ), {"customer": customer_id, "company": company}, as_dict=True)[0].total or 0
        
        # Get advance credits total
        advance_credit = frappe.db.sql("""
