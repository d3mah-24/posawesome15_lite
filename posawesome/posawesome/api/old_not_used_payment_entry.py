...
def process_pos_payment(payload):
    try:
        frappe.logger().info('[payment_entry.py] process_pos_payment called')
        data = json.loads(payload)
        data = __dict(data)
        # External payments functionality disabled - no validation needed
        # validate data
        if not data.customer:
            frappe.logger().error('[payment_entry.py] Customer is required')
            frappe.throw(_("Customer is required"))
        if not data.company:
            frappe.logger().error('[payment_entry.py] Company is required')
            frappe.throw(_("Company is required"))
        if not data.currency:
            frappe.logger().error('[payment_entry.py] Currency is required')
            frappe.throw(_("Currency is required"))
        if not data.pos_profile_name:
            frappe.logger().error('[payment_entry.py] POS Profile is required')
            frappe.throw(_("POS Profile is required"))
        if not data.pos_opening_shift_name:
            frappe.logger().error('[payment_entry.py] POS Opening Shift is required')
        ...
        # External payments functionality disabled - no new payments processing
        if False:  # Disabled external payments
            for payment_method in data.payment_methods:
                try:
                    if not payment_method.get("amount"):
                        continue
                    frappe.logger().info(f'[payment_entry.py] Creating payment entry: {payment_method}')
                    ...
                    frappe.logger().error(f'[payment_entry.py] Error creating payment entry: {e}')
                    errors.append(e)

        # External payments reconciliation disabled
        if False:  # Disabled external payments reconciliation
                # add the unallocated payments to the all payments entry
                for selected_payment in data.selected_payments:
                    frappe.logger().info(f'[payment_entry.py] Adding selected payment to reconciliation: {selected_payment.get("name")}')
                    all_payments_entry.append(selected_payment)

            if len(all_payments_entry) > 0:
                # sort the all payments entry by posting date
                frappe.logger().info('[payment_entry.py] Reconciling payments and invoices')
        ...
        if len(data.selected_invoices) > 0 and data.total_selected_invoices > 0:
            ...
            msg += "</table>"
        if len(errors) > 0:
            frappe.logger().error(f'[payment_entry.py] Errors occurred: {errors}')
            msg += "<h4>Errors</h4>"
            msg += "<table class='table table-bordered'>"
        ...
        frappe.logger().error(f'[payment_entry.py] Error in process_pos_payment: {e}')
        raise
